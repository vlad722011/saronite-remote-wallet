...
#             + os.path.join(self.app.property("ResPath"), "www", self.html).replace('\\', '/')
        ...
        self.icon = self._getQIcon('saronite_64x64.png')
...
class MainWebUI(BaseWebUI):
    def __init__(self, app, hub, debug):
        ...
        if sys.platform == 'darwin':
            tray_icon = 'saronite_16x16_mac.png'
        elif sys.platform == "win32":
            tray_icon = 'saronite_16x16.png'
        else:
            tray_icon = 'saronite_32x32_ubuntu.png'
        ...
        self.saronited_daemon_manager = None
...
#         self.view.load(qt_core.QUrl(self.url))
    ...
    def run_wallet_rpc(self, wallet_password, log_level=1):
        ...
        while True:
            ...
            saronited_info = self.daemon_rpc_request.get_info()
            if saronited_info['status'] == "OK":
    ...
    def _update_daemon_status(self):
        ...
        if saronited_info['status'] == "OK":
            status = "Connected"
            self.current_height = int(saronited_info['height'])
            target_height = int(saronited_info['target_height'])
        ...
        sync_status = "Disconnected" if saronited_info['status'] != "OK" else "Synchronizing..."
        if saronited_info['status'] == "OK" and self.current_height == self.target_height:
